...
const TablaEmpresa = () => {
  ...
  const Campos = ["Nombre", "Total"];

  useEffect(() => {
    //Por reemplazar
    db.collection("empresasOrd")
      .get()
      .then((snapshot) => {
        const datos = snapshot.docs.map((doc) => doc.data());
        setValores(datos);
        console.log("HEMOS LEÍDO DATOS: ", datos);
      });
  }, []);

  return (
    <CCol style={{ marginTop: "40px" }}>
      <CCard>
        <CCardHeader>Instituciones gubernamentales</CCardHeader>
        <CCardBody>
          <CDataTable
            items={valores}
            fields={Campos}
